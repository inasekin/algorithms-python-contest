...
with open("input.txt", "r") as file:
    for line in file:
        ...
        if operation == "DEPOSIT":
            name, amount = args
            balances[name] = balances.get(name, 0) + int(amount)
            ...
            from_name, to_name, amount = args
            balances[from_name] = balances.get(from_name, 0) - int(amount)
            balances[to_name] = balances.get(to_name, 0) + int(amount)
        elif operation == "INCOME":
            percent = int(args[0])
            for name in balances:
                if balances[name] > 0:
                    balances[name] += balances[name] * percent // 100